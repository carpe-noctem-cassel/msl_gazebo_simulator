...
void NubotGazebo::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
	...
	{
		ROS_FATAL_STREAM(
				"A ROS node for Gazebo has not been initialized, unable to load plugin. " << "Load the Gazebo system plugin 'libnubot_gazebo.so' in the gazebo_ros package)");
		return;
	}
	...
	ros::SubscribeOptions so1 = ros::SubscribeOptions::create<gazebo_msgs::ModelStates>(
			"/gazebo/model_states", 100, boost::bind(&NubotGazebo::model_states_CB, this, _1), ros::VoidPtr(),
			&this->message_queue_);
	...
	ros::SubscribeOptions so2 = ros::SubscribeOptions::create<nubot_common::VelCmd>(
			"nubotcontrol/velcmd", 100, boost::bind(&NubotGazebo::vel_cmd_CB, this, _1), ros::VoidPtr(),
			&this->message_queue_);
	this->Velcmd_sub_ = this->rosnode_->subscribe(so2);

	ros::SubscribeOptions so3 = ros::SubscribeOptions::create<msl_actuator_msgs::MotionControl>(
				"MotionControl", 100, boost::bind(&NubotGazebo::cnc_vel_cmd_CB, this, _1), ros::VoidPtr(),
				&this->message_queue_);
...
//	reconfigureServer_->setCallback(boost::bind(&NubotGazebo::config, this, _1, _2));
	...
	this->update_connection_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&NubotGazebo::UpdateChild, this));
}
...
void NubotGazebo::config(nubot_gazebo::NubotGazeboConfig &config, uint32_t level)
{
	...
	ROS_INFO("Reconfig request: P:%f I:%f D:%f I_term_max:%f I_term_min:%f", dribble_P_, dribble_I_, dribble_D_,
				I_term_max_, I_term_min_);
...
void NubotGazebo::cnc_vel_cmd_CB(const msl_actuator_msgs::MotionControl::ConstPtr& mc)
{
	...
	Vy_cmd_ = sin(mc->motion.angle+M_PI)*(mc->motion.translation/1000.0);
	w_cmd_ = mc->motion.rotation;
...
bool NubotGazebo::get_trans_vector(math::Vector3 target_point_world, double metre_thres)
{
	...
	{
		ROS_INFO("last_target:%f %f, target_point:%f %f", last_target_point.x, last_target_point.y,
					target_point_world.x, target_point_world.y);
		if ((last_target_point - target_point_world).GetLength() > 0.5) // CAN TUNE
		{
			last_current_time = world_->GetSimTime();
			traj_plan_linear_.Init(traj_const_lin_accel, max_linear_vel_, nubot_target_vector_length - metre_thres);
		}
		//double _dt = (world_->GetSimTime() - last_current_time).Double();
		double _dt = 0.007;
...
void NubotGazebo::ball_vel_decay(math::Vector3 vel, double mu)
{
	...
	if (vel_len > 0.0)
	{
		if (football_state_.pose.position.z <= 0.12 && !(last_vel_len - vel_len > 0))
		{
